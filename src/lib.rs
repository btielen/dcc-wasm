...
pub fn parse(dcc_certificate: &str) -> ParsingResult {
    ...
    match dcc_certificate.strip_prefix("HC1:") {
        Some(stripped) => dcc_certificate = stripped,
        None => ()
    }
    ...
    match decompressor.read_to_end(&mut decompressed) {
        Ok(_result) => (),
        Err(e) => return ParsingResult::create_failure(&format!("Error on zlib decompressing: {}", e))
    }
